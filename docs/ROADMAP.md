...
|-----|---------|----------------|----------|
| `async` keyword unused | Grammar still lexes `async` (`crates/otterc_parser/src/grammar.rs:81-109`) but no AST nodes/type rules consume it. Decide to remove keyword or implement async functions + type checker rules. | Parser, `src/typecheck/checker.rs` | High |
...
| Type checker modularity & diagnostics | `TypeChecker::check_program` rebuilds the entire program and stores everything in a single global context on each run (`src/typecheck/checker.rs:520-790`). Module graphs, cross-file `use`/`pub use` resolution, and incremental re-checks are missing, which also limits LSP cross-file support. We need persistent symbol tables, better error aggregation, and per-module diagnostics so the CLI/LSP can show multiple issues at once. | Type checker, LSP | Medium |
...
| Type checker unaware of task handles | `await` always returns `unit` and `spawn` returns `Opaque` (`src/typecheck/checker.rs:2142-2164`). Once the runtime defines task-result APIs, propagate the types so user code can reason about handle payloads. | Type checker, runtime | High |
...
| Task runtime feature gating | Async-aware sleep/waker paths are hidden behind `#[cfg(feature = "task-runtime")]` (`src/runtime/stdlib/task.rs:1-74`), yet the CLI exposes `--tasks*` flags (`src/cli.rs:16-80`). Either ship the feature by default or error when the flag is set without the feature enabled. | Task stdlib, CLI | Medium |
| Channel performance & semantics | `TaskChannel` uses `Vec::remove(0)` (O(n)) and duplicates wakers when channels close (`src/runtime/task/channel.rs:33-167`). Switch to a deque/ring buffer, add tests for close-before-send, and ensure metrics record backlog correctly. | Task channel | Medium |
